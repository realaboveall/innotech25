...
const AuthStep = ({ onSignIn }) => (
    <motion.div key="auth" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.5 }}>
        <GlassSection className="max-w-md mx-auto text-center">
            <h1 className="text-3xl font-semibold mb-4 text-white">Join the Innovation</h1>
            <p className="text-gray-400 mb-8">Begin by securely signing in with your Google account.</p>
            <button onClick={onSignIn} className="w-full flex items-center justify-center gap-3 bg-white/90 text-black font-semibold py-3 px-6 rounded-lg hover:bg-white transition-all duration-300 shadow-lg hover:shadow-cyan-400/30">
...
const CategoryStep = ({ onSelect }) => {
    const cards = [
        { title: "School", icon: "🎓", desc: "For school students showcasing creative innovation projects." },
        { title: "College", icon: "🏫", desc: "For college innovators ready to present research and prototypes." },
        { title: "Researcher", icon: "🔬", desc: "For academic and institutional researchers driving discovery." },
        { title: "Startup", icon: "🚀", desc: "For startup founders transforming ideas into impact." },
    ];
    return (
        <motion.div key="category" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.5 }}>
            <GlassSection className="max-w-4xl mx-auto text-center">
                <h1 className="text-4xl md:text-5xl font-semibold mb-6 bg-gradient-to-r from-cyan-300 via-purple-300 to-pink-300 bg-clip-text text-transparent">Register As</h1>
                <p className="text-gray-300 text-lg md:text-xl mb-12 max-w-3xl mx-auto">Choose your category to proceed.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
                    {cards.map((card, i) => (
                        <motion.div key={card.title} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} onClick={() => onSelect(card)} className="group relative bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-8 flex flex-col justify-center items-center text-center cursor-pointer hover:border-white/20 hover:shadow-[0_0_15px_rgba(255,255,255,0.08)] transition-all duration-500">
                            ...
                            <div className="relative z-10 flex flex-col items-center">
                                <div className="text-4xl mb-4 text-white/80 group-hover:text-white transition-all duration-300">{card.icon}</div>
                                <h2 className="text-xl font-semibold mb-2 text-white tracking-wide group-hover:text-cyan-300 transition-colors duration-300">{card.title}</h2>
                                <p className="text-gray-400 text-sm leading-relaxed max-w-xs">{card.desc}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
...
const BasicInfoStep = ({ formData, handleFormChange, onSubmit, onBack, selectedCategory }) => {
    return (
        <motion.div key="basic-info" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.5 }}>
            <GlassSection className="max-w-xl mx-auto">
                <h1 className="text-3xl font-semibold mb-2 text-center text-white">Basic Information</h1>
                <p className="text-gray-400 mb-8 text-center">You're registering as <span className="text-cyan-300 font-semibold">{selectedCategory?.title}</span>. Let's get the final details.</p>
...
const App = () => {
    ...
    const [formData, setFormData] = useState({
        name: "", phonenumber: "", isKietian: false, participationCategory: "",
    });
    ...
    try {
      window.location.href = "https://2q766kvz-8001.inc1.devtunnels.ms/auth/google";
  ...
  // On mount, extract token from URL (query, hash or href) and save it, then proceed to Category step
    useEffect(() => {
      const tryExtractToken = () => {
        ...
        try {
          ...
          try { saveTokenToCookie(token, { expires: 7 }); } catch (e) { console.warn('save cookie failed', e); }
          ...
          } catch (e) { console.warn('failed to clean URL', e); }
          ...
          (async () => {
            ...
            if (!token) return;
            try {
              const res = await fetch('https://2q766kvz-8001.inc1.devtunnels.ms/api/user/profile', {
                headers: {
    ...
    const handleCategorySelect = (card) => {
        ...
        setFormData(prev => ({ ...prev, participationCategory: card.title.toLowerCase() }));
        setStep(3);
    };
    ...
    const handleFinalFormSubmit = async (e) => {
        ...
        try {
          const res = await fetch('https://2q766kvz-8001.inc1.devtunnels.ms/api/user/complete-profile', {
            method: 'PUT',
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
